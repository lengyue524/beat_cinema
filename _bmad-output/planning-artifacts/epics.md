...
- S10.10: 歌曲列表多选与批量右键操作
- S10.11: Playlist 详情高级管理操作（删除/添加/移动）
- S10.12: 通用歌单选择弹窗（支持搜索）
...
**用户故事**：作为用户，我希望在歌曲列表中支持多选，并对选中项执行批量右键操作（添加到歌单、删除歌曲目录），以减少重复操作成本。

**验收标准**：
- [ ] 歌曲列表支持多选（Ctrl/Shift 区间选择）与清空选择
- [ ] 多选状态下右键菜单展示批量操作：添加到歌单、删除歌曲目录
- [ ] 单选状态下仍保留现有右键常用操作，不影响原流程
- [ ] 批量删除前二次确认，并明确显示将删除的条目数量
- [ ] 删除完成后列表状态与统计即时更新，失败项可见反馈

**依赖**：S2.4, S5.3, S8.3  
**依赖标注（沿用 S10.3 标准）**：
- 依赖 Story：`2-4-level-list-tile`, `5-3-context-menu-region`, `8-3-playlist-video-status`
- 阻塞 Story：`10-11-playlist-detail-cross-playlist-operations`
- 解锁条件：列表可稳定维护多选状态并完成批量操作闭环
- 依赖类型：mandatory
- 风险等级：medium
**估算**：5 SP
...
**用户故事**：作为用户，我希望在 Playlist 详情中对歌曲执行删除、添加到其他歌单、移动到其他歌单等操作，并能在删除时选择是否同步删除歌曲文件目录。

**验收标准**：
- [ ] Playlist 详情支持单项/多项删除操作
- [ ] 删除操作支持开关：仅移除歌单项 / 同步删除歌曲目录
- [ ] 支持“添加到歌单”与“移动到其他歌单”操作
- [ ] 移动操作完成后，源歌单与目标歌单条目数量与状态保持一致
- [ ] 文件删除失败不应导致歌单数据损坏，需给出明确提示

**依赖**：S8.2, S8.3, S4.4  
**依赖标注（沿用 S10.3 标准）**：
- 依赖 Story：`8-2-playlist-browse-page`, `8-3-playlist-video-status`, `4-4-atomic-file-write-service`
- 阻塞 Story：`10-12-playlist-picker-dialog-with-search`
- 解锁条件：Playlist 详情完成“删/加/移”端到端闭环且可回滚异常
- 依赖类型：mandatory
- 风险等级：high
**估算**：5 SP
...
**用户故事**：作为用户，我希望在添加或移动歌曲到歌单时，通过一个可搜索的歌单选择弹窗快速定位目标歌单并完成操作。

**验收标准**：
- [ ] 提供统一歌单选择弹窗，支持搜索过滤
- [ ] 搜索支持大小写不敏感与关键词高亮（可选）
- [ ] 可禁用当前歌单作为移动目标（避免无效操作）
- [ ] 目标歌单为空时提供空状态提示
- [ ] 弹窗可复用于“添加到歌单”“移动到歌单”两类场景

**依赖**：S5.1, S5.2, S10.11  
**依赖标注（沿用 S10.3 标准）**：
- 依赖 Story：`5-1-panel-host`, `5-2-panel-cubit`, `10-11-playlist-detail-cross-playlist-operations`
- 阻塞 Story：无
- 解锁条件：歌单选择器可在两类入口稳定复用并完成搜索选择
- 依赖类型：mandatory
- 风险等级：medium
**估算**：3 SP
